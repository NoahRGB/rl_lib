...
from rl_lib.utils.networks import ICM_ActorCriticNetwork
...
class ICM_PPO:
    ...
    def __init__(self, lr: float, tmax: int, gamma: float, lam: float, 
                 ...
                 eta: float, beta: float, icm_lam: float, load_path: str = None):
        self.lr = lr
        self.tmax = tmax
        self.gamma = gamma
        self.lam = lam
    ...
    def setup(self, env: EnvDetails, device: torch.device) -> None:
        ...
        self.network = ICM_ActorCriticNetwork(env, self.net_architecture).to(device)
    ...
    def learn(self):
        ...
        with torch.no_grad():
            _, state_values = self.network.actor_critic_out(full_batch.states.view(-1, *self.env.state_space.shape)) # (tmax, num_envs, 1)
            _, final_state_values = self.network.actor_critic_out(full_batch.next_states[-1].view(-1, *self.env.state_space.shape)) # (tmax, num_envs, 1)
        ...
        for epoch in range(self.epochs):
            ...
            for minibatch_start in range(0, total_batch_size, self.minibatch_size):
                ...
                if self.is_continuous:
                    (minibatch_mu, minibatch_sigma), minibatch_state_values = self.network.actor_critic_out(minibatch.states)
                    ...
                    minibatch_logits, minibatch_state_values = self.network.actor_critic_out(minibatch.states)
                ...
                action_pred = self.network.inv_model(minibatch.states, minibatch.next_states)
                inv_model_loss = torch.nn.functional.cross_entropy(action_pred, minibatch.actions.squeeze(-1).long())

                phi_next_state, phi_next_state_pred = self.network.forward_model(minibatch.states, minibatch.actions, minibatch.next_states)
                forward_loss = 0.5 * torch.nn.functional.mse_loss(phi_next_state_pred, phi_next_state.detach(), reduction="none").sum(-1).mean()

                total_loss = self.icm_lam * combined_loss + (1 - self.beta) * inv_model_loss + self.beta * forward_loss
    ...
    def act(self, state) -> Step:
        with torch.no_grad():
            actor_out, _ = self.network.actor_critic_out(torch.from_numpy(state).float().to(self.device))
    ...
    def timestep_complete(self, timestep: int, state, step: Step, reward, next_state, done) -> None:
        ...
        with torch.no_grad():
            forward_model_out = self.network.forward_model(
                torch.from_numpy(state).to(self.device),
                torch.from_numpy(step.action).to(self.device),
                torch.from_numpy(next_state).to(self.device)
            )

            phi_next_state, phi_next_state_pred = forward_model_out
            squared_l2_norm = torch.nn.functional.mse_loss(phi_next_state, phi_next_state_pred, reduction="none").sum(-1)
            intrinsic_reward = (self.eta / 2) * squared_l2_norm

        self.buffer.add(state, step.action, reward, next_state, done, step.log_prob, intrinsic_reward.cpu().numpy())
